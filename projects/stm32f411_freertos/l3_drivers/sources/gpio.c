...
{
  exti__select_interrupt_line_source(gpio.pin, (uint8_t)gpio.port);
  exti__setup_external_interrupt(gpio.pin, (exti__trigger_edge_e)edge);
  exti__enable_external_interrupt(gpio.pin);
}